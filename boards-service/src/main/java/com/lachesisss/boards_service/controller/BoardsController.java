...
public class BoardsController {
	...
	@GetMapping(value = "/boards/{id}/lists")
	public ResponseEntity<?> getListsByIdBoard(@PathVariable String id) {
		try {
			Boards board;
			board = service.getBoardById(id).get();
			...
			List<Object> lists = restTemplate.getForObject(uri, List.class);
	...
	public ResponseEntity<?> createListByIdBoard(@PathVariable String id) {
		try {
			...
			final String uri = "http://localhost:8081/lists";
